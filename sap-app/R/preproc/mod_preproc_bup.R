...
mod_preproc_server <- function(id, data_global) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      # CORREÇÃO: Selecionar variáveis padrão automaticamente
      ...
      updateSelectizeInput(session, "vars_keep", choices = vnum, selected = selected_vars)
    })
    ...
    output$n_vars <- renderText({
      vars <- vars_sel()
      req(length(vars) > 0)  # CORREÇÃO: Adicionar req()
    ...
    output$p_corr <- renderPlotly({
      v <- vars_sel()
      req(length(v) >= 2)  # CORREÇÃO: Adicionar req()
