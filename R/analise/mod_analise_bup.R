...
criar_sistema_filtros_modal <- function(session, ns, id, results_reactive, filtros_aplicados) {
  ...
  observeEvent(session$input[[paste0("btn_abrir_modal_", id)]], {
    ...
    # Usar isolate para evitar loops
    isolate({
  ...
  observeEvent(session$input[[paste0("btn_adicionar_filtro_", id)]], {
    ...
    # Capturar valores reativos ANTES
  ...
  page_sidebar(
    ...
    navset_card_tab(
      ...
      nav_panel(
        ...
        # WRAPPER COM ROLAGEM (igual à aba Qualificação)
        div(
...
mod_analise_server <- function(id, preproc_data) {
  moduleServer(id, function(input, output, session) {
    ...
    # ====================================================================
    # PERFIS MANUAIS: MODAL COM HISTOGRAMAS ESTÁTICOS E CLIQUES
    # ====================================================================
    
    # ReactiveValues para armazenar perfis definidos manualmente
    # ====================================================================
    # PERFIS MANUAIS: MODAL COM HISTOGRAMAS INTERATIVOS (CORRIGIDO)
    # ====================================================================
    ...
    observe({
      ...
      if (is.null(n_classes) || n_classes < 3 || n_classes > 7) {
        n_classes <- 5
      }
      ...
      for (cn in crits) {
        # Só inicializar se ainda não existe
    ...
    # Botão para abrir modal de perfis interativos
    observeEvent(input$btn_abrir_modal_perfis, {
      ...
      showModal(modalDialog(
        ...
        div(
          ...
          # Gerar UI para cada critério (layout 2 colunas)
          lapply(seq(1, length(crits), by = 2), function(idx) {
            # Pegar até 2 critérios por linha
              ...
              lapply(crits_linha, function(cn) {
                ...
                column(
                  ...
                  wellPanel(
                    ...
                    # Inputs numéricos para os perfis
                    tags$div(
    ...
    # Renderizar histogramas com ggplot2 (FORA do observe)
    observe({
      req(input$criterios_sel)
      req(length(input$criterios_sel) > 0)
      ...
      # Criar outputs para cada critério
      lapply(crits, function(cn) {
        output[[paste0("hist_perfil_", cn)]] <- renderPlot({
          ...
          # Valores atuais dos perfis (REAIS)
    ...
    # Observar cliques nos histogramas (DIRETO, sem local)
    observe({
      req(input$criterios_sel)
      req(length(input$criterios_sel) > 0)
      ...
      # Criar observeEvent para cada critério
      lapply(crits, function(cn) {
        observeEvent(input[[paste0("click_perfil_", cn)]], {
          ...
          # Atualizar o perfil clicado
          vals_real[idx] <- x_clicado
          
          # IMPORTANTE: Reordenar via normalização [0,1]
          sense <- input[[paste0("sense_", cn)]] %||% "benefit"
    ...
    # Observar mudanças diretas nos inputs numéricos
    observe({
      req(input$criterios_sel)
      req(length(input$criterios_sel) > 0)
      ...
      lapply(crits, function(cn) {
        # Para cada perfil
        lapply(1:n_perfis, function(i) {
          observeEvent(input[[paste0("perfil_modal_", cn, "_", i)]], {
            valor <- input[[paste0("perfil_modal_", cn, "_", i)]]
            
            if (!is.null(valor) && is.numeric(valor)) {
              # Pegar todos os valores atuais
              vals_real <- get_perfis_vals_real(cn, n_perfis)
              vals_real[i] <- valor
              ...
              # Se precisar reordenar, atualizar os inputs
              if (!isTRUE(all.equal(vals_real, vals_real_sorted))) {
                for (j in 1:n_perfis) {
                  updateNumericInput(session, paste0("perfil_modal_", cn, "_", j), 
                                     value = round(vals_real_sorted[j], 4))
                }
              }
            }
          }, ignoreInit = TRUE)
    ...
    # Botão de reset individual por critério
    observe({
      req(input$criterios_sel)
      req(length(input$criterios_sel) > 0)
      ...
      lapply(crits, function(cn) {
        observeEvent(input[[paste0("reset_perfil_", cn)]], {
          ...
          showNotification(
            paste0("Perfis de '", cn, "' resetados para quantis padrão"),
            type = "warning",
            duration = 2
          )
        })
    ...
    # Botão para resetar TODOS os perfis
    observeEvent(input$btn_resetar_perfis, {
      ...
      showNotification("Todos os perfis resetados para quantis padrão", 
                       type = "warning", duration = 3)
    })
    
    # Botão para salvar perfis e fechar modal
    observeEvent(input$btn_salvar_perfis, {
      removeModal()
      showNotification("Perfis salvos com sucesso!", type = "message", duration = 3)
    })
    
    # Mostrar resumo dos perfis definidos
    ...
    B_current <- reactive({
      ...
      if (input$b_mode == "quantis") {
        ...
        for (j in seq_along(crits)) {
          ...
          # Obter valores reais dos perfis manuais
    ...
    criar_sistema_filtros_modal(
      session = session, ns = ns, id = "tabela",
      results_reactive = reactive({
        req(resultados_electre())
        resultados_electre()$results
      }),
    ...
    # ====================================================================
    # ABA 5: QUALIFICAÇÃO - DADOS ESPACIAIS E INTERSEÇÕES
    # ====================================================================
    ...
    dados_espaciais <- reactive({
      ...
      tryCatch({
        ...
        list(quilombolas = lista_sf[[1]], assentamentos = lista_sf[[2]],
             indigenas = lista_sf[[3]], ensino = lista_sf[[4]],
             prisoes = lista_sf[[5]], sementes = lista_sf[[6]])
      }, error = function(e) {
        showNotification(paste("Erro ao carregar dados espaciais:", e$message),
    ...
    criar_sistema_filtros_modal(
      session = session, ns = ns, id = "qualificacao",
      results_reactive = reactive({
      ...
      withProgress(message = 'Processando interseções espaciais...', value = 0, {
        ...
        if (!is.null(dados$ensino)) {
          ...
          inter_list$ensino <- tryCatch({
            suppressWarnings({ ensino_pontos <- st_centroid(dados$ensino) })
    ...
    output$tab_ranking_municipios <- renderDT({
      ...
      contagens_municipio <- data.frame(CD_MUN = character(), NM_MUN = character(),
                                        Quilombolas = integer(), Assentamentos = integer(),
                                        T_Indigenas = integer(), Inst_Ensino = integer(),
                                        U_Prisionais = integer(), B_Sementes = integer(),
                                        Total = integer(), stringsAsFactors = FALSE)
      ...
      for (i in 1:nrow(resultado_sf)) {
        ...
        total <- sum(counts)
        if (total > 0) {
          contagens_municipio <- rbind(contagens_municipio,
                                       data.frame(CD_MUN = cd_mun, NM_MUN = nm_mun,
                                                  Quilombolas = counts[1], Assentamentos = counts[2],
                                                  T_Indigenas = counts[3], Inst_Ensino = counts[4],
                                                  U_Prisionais = counts[5], B_Sementes = counts[6],
                                                  Total = total, stringsAsFactors = FALSE))
        }
      }
      
      contagens_municipio <- contagens_municipio[order(-contagens_municipio$Total), ]
      contagens_municipio <- cbind(Ranking = 1:nrow(contagens_municipio), contagens_municipio)
      ...
      datatable(contagens_municipio, rownames = FALSE,
    ...
    output$export_mapa_qualif <- downloadHandler(
      ...
      content = function(file) {
        ...
        # Usar o mesmo código do mapa acima
        resultado_sf <- resultado_qualif_filtrado()
        inter <- intersecoes()
        n_classes <- resultados_electre()$params$n_classes %||% 5
    ...
    # ====================================================================
    # RETORNAR DADOS PARA OUTROS MÓDULOS (ex: Relatórios)
    # ====================================================================
    
    return(
      list(
        # Resultados da análise ELECTRE
        resultados_electre = reactive({
          req(resultados_electre())
          resultados_electre()
        }),
        
        # Dados de qualificação territorial (interseções)
        intersecoes = reactive({
          # Retornar NULL se ainda não calculou as interseções
          if (is.null(intersecoes())) {
            return(NULL)
          }
          intersecoes()
        }),
        
        # Data frame espacial original
        data_sf = reactive({
          req(data_sf())
          data_sf()
        }),
